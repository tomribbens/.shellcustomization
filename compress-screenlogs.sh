...
# live screen window isn't still writing to, so it's safe to run repeatedly
# (e.g. from .bash_profile on every login): an active log is skipped (its window
# still holds the file open) and picked up on a later run once the window closes.
#
